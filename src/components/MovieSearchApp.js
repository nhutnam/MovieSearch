...
// https://api.themoviedb.org/3/search/movie?api_key=c713845790a8c1484cc0a0505634473c&query=%27%27
const MovieSearchApp = () => {
    ...
    useEffect(() => {
        async function fetchData() {
            setLoading(true);
            const response = await axios.get(`https://api.themoviedb.org/3/search/movie?api_key=c713845790a8c1484cc0a0505634473c&query='${queryDebounce}'`);
            //3.49
    ...
    return (
        <div className='p-10'>
            <div className="w-full object-cover rounded-lg mb-20 max-w-[500px] mx-auto">
                <input type="text" 
                        className='w-full shadow-[0px_0px_0px_3px_rgba(125,_106,_255,_0.2)] p-5 outline-none rounded-lg border border-purple-500' 
    ...
    return(
        <div className='bg-white p-3 rounded-2xl shadow-sm flex flex-col'>
            <div className='h-[297px]'>
                <LoadingSkeleton radius="16px" width="100%" height="100%"></LoadingSkeleton>
            </div>
...
const MovieItem = ({data}) => {
    return (
        <div className='bg-white p-3 rounded-2xl shadow-sm flex flex-col'>
            ...
            <div className="p-7 flex-1 flex-col">
                <h3 className='mb-4 text-lg text-black font-semibold'>{data.title}</h3>
                <p className='text-[#999] text-sm mb-6 !leading-relaxed'>
                    {data.overview}
                </p>
                <div className="flex items-center gap-x-3 mt-auto">
                    ...
                    <span className='text-sm text-[#333] font-semibold'>{data.vote_average}</span>
                </div>
            </div>
        </div>
    )
